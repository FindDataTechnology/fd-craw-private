...
Defines the in-app Settings modal: a single overlay surface that hosts the application's configuration sections (General, Models, MCP, Skills, System Status), its `/settings/:section` routing and deep-link behavior, its open and dismiss affordances, and the redirects that retire the former standalone configuration pages.
