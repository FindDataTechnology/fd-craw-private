...
- [ ] 1.1 Logto: create groups `legal` and `analysts`; assign the demo customer accounts. Verify: Logto admin console shows members; a test login returns the group in claims. *(平台侧需要 organization + organization_role（`logto-auth.js` 读 `organizations`/`organization_roles` claim）；registry 侧已用 idp_user_groups fallback 替代，见 §3.1 补充)*
...
- [ ] 2.5 Register all four skills into the registry (admin UI or `POST` skill API on cheap1), tags aligned to pack domain. Verify: `GET /api/skills?limit=500` lists each; platform Store shows them to the mapped role. *(端点已确认 `POST /api/skills`，需 `skill_md_url`——托管位置待定：公开 GitHub 仓库 / gitee raw + token / 内网静态服务；内容无密钥但是否公开需拍板)*

## 3. Conversational agents (cloud catalog, zero platform code)

- [ ] 3.1 Author the cloud agents document (合同审查官 / 案件分析师 / 行业分析师): `agent-remote` chat entries, `baseUrl: https://token.finddatatech.cloud/v1`, `model: deepseek-v4-pro`, `apiKeyEnv: FD_TOKEN_API_KEY`, roles mapped per pack. Verify: document contains no inline secret; `GET /api/catalog` returns all three after refresh.
- [ ] 3.2 fd-prod: set `FD_TOKEN_API_KEY` secret + `AGENTS_CONFIG_URL` config; rollout. Verify: chatting with each agent streams a reply through the endpoint (Trace page shows the upstream model call).
